...
export function DataTableToolbar<TData>({ table }: DataTableToolbarProps<TData>) {
  ...
  return (
    <div className="flex gap-1 dark:text-white items-center">
          ...
          <Input className="max-w-xs" placeholder="isPaid filter..."
              value={(table.getColumn("isPaid")?.getFilterValue() as string) ?? ""}
              onChange={(event) => table.getColumn("isPaid")?.setFilterValue(event.target.value)}                
          />
