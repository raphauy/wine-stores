...
export function DataTableToolbar<TData>({ table }: DataTableToolbarProps<TData>) {
  ...
  return (
    <div className="flex gap-1 dark:text-white items-center">
          ...
          <Input className="max-w-xs" placeholder="igHandle filter..."
              value={(table.getColumn("igHandle")?.getFilterValue() as string) ?? ""}
              onChange={(event) => table.getColumn("igHandle")?.setFilterValue(event.target.value)}                
          />
